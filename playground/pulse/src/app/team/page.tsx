...
const DAY_LABELS = { lun: 'Lundi', mar: 'Mardi', mer: 'Mercredi', jeu: 'Jeudi', ven: 'Vendredi' }
const REASON_CONFIG = {
...
export default async function TeamPage() {
    ...
    const absentToday = absences.filter(a => a.days.includes('mer')).length

    return (
        <Shell title="Congés / Équipe" subtitle={`Semaine 21 · ${absentToday} absent${absentToday > 1 ? 's' : ''} aujourd'hui`}>
            <div className="tl">
                <h2 className="tl-title">Planning de la semaine</h2>
                <div className="tl-grid">
                    <div className="tl-header-row">
                        <div />
                        {DAYS.map(d => <div key={d} className="tl-daylabel">{DAY_LABELS[d]}</div>)}
                    </div>
                    {TEAM.map(person => {
                            ...
                            <div key={person} className="tl-row">
                                ...
                                {DAYS.map(day => {
                                    ...
                                    return (
                                        <div
                                            key={day}
                                            className={`tl-cell ${isAbsent ? 'tl-absent' : 'tl-present'}`}
            ...
            <style>{`
                ...
                .tl-daylabel { font-size: 0.6875rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: var(--muted); text-align: center; padding: 0.25rem 0; }
